  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editingCliente) {
        ...
        // Verificar si existe un cliente con la misma cédula (activo o inactivo)
        ...
        const { data: clientesExistentes } = await checkQuery

        if (clientesExistentes && clientesExistentes.length > 0) {
          const clienteActivo = clientesExistentes.find(c => c.estado === true)
          const clienteInactivo = clientesExistentes.find(c => c.estado === false)

          if (clienteActivo) {
            // Ya existe un cliente activo con esta cédula
            alert('Ya existe un cliente activo con esta cédula.')
            return
          }

          if (clienteInactivo) {
            // Reactivar el cliente inactivo con los nuevos datos
            const { error } = await supabase
              .from('clientes')
              .update({
                ...clienteData,
